...
//  });
	...
	public int performRead(int address, long cycles) {
		...
		case 0xdd00:
			...
			val = (cia2PRA | ~cia2DDRA) & 0x3f | iecLines & c1541Chips.iecLines;
...
//    Integer.toHexString(data));
	...
	private void printIECLines() {
		...
		System.out.println(" => C" + ((iecLines & c1541Chips.iecLines & 0x80) == 0 ? 1 : 0) + " D"
		...
		case 0:
			...
			if ((irqFlags & 1) == 0 && (irqComp == vbeam)) {
				...
				if ((irqMask & 1) != 0) {
					...
					lastIRQ = cpu.cycles;
					if (IRQDEBUG)
						monitor.info("Generating IRQ at " + vbeam + " req:" + raster + " IRQs:" + cpu.interruptInExec
...
//      System.out.println("Cycle: " + vicCycle + " VMLI: " + vmli + " => " + mpos);
	 ...
	 *
	 *
	 * @param mpos an <code>int</code> value representing position to draw the
...
//     repaint = System.currentTimeMillis();
	...
	public void mousePressed(MouseEvent e) {
		System.out.println("c64screen . mousePressed");
